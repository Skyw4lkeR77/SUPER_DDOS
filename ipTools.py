...
""" Memeriksa apakah situs web berada di bawah Proteksi CloudFlare """
    ...
    try:
        ...
        for i in range(len(ipv4)):
            if ipaddress.ip_address(origin) in ipaddress.ip_network(ipv4[i]):
                print(
                    f"{Fore.RED}[!] {Fore.CYAN}Situs web ini dilindungi oleh CloudFlare, serangan ini mungkin tidak memberikan hasil yang diinginkan.{Fore.RESET}"
                )
                sleep(1)
